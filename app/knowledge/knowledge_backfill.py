...
# The warm path needs transcripts held in memory by the process that produced
# them, which live in the API layer. Rather than importing downward-to-upward,
# that layer registers a lookup here at import time — knowledge stays a library
# that upper layers call, never one that calls them (see tests/test_layering.py).
_warm_segment_source: Optional[Callable[[str], Optional[tuple[list[dict], Optional[str]]]]] = None


def register_warm_segment_source(
    lookup: Optional[Callable[[str], Optional[tuple[list[dict], Optional[str]]]]],
) -> None:
    """Supply in-process transcript segments. Pass None to clear (tests)."""
    global _warm_segment_source
    _warm_segment_source = lookup
...
def resolve_segments_for_job(
    ...
    segments.
    """
    # Warm path — whatever an upper layer registered, if anything.
    if _warm_segment_source is not None:
        try:
            warm = _warm_segment_source(job_id)
            if warm and warm[0]:
                return warm
