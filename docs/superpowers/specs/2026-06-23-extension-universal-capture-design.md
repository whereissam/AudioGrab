...
**Manifest changes (`manifest.json` + `manifest.firefox.json`):**
- `permissions`: add `webRequest`, `downloads`. Keep `activeTab`, `storage`.
- `host_permissions`: broaden to `<all_urls>` (required for sniffing + content script everywhere) **plus** the user's Sift server origin.
...
- **Extension (pure-function unit tests, runnable with `bun test`):**
